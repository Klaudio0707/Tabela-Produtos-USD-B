...
router.post("/login", async (req, res) => {
    ...
    try {
        ...
        if (!user) {
            return res.status(401).json({ message: "Usuário não Cadastrado ou não Encontrado" })
...
router.post('/register', async (req, res) => {
    ...
    try {
        ...
        res.status(500).json({ message: "Erro no servidor. Não foi possível registrar o usuário.", error: error.message });
    }
});
