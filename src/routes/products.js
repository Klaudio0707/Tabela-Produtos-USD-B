...
router.post("/", async (req, res) => {
  const { name, manufacturer, origin, package, price, currency } = req.body;
  try {
...
router.put("/:id", async (req, res) => {
  ...
  if (!name || !manufacturer || !price || !currency) {
    return res
      .status(400)
      .json({ message: "Dados insulficientes para atualizar o produto." });
  }
  const updates = {
    name,
    manufacturer,
  ...
  try {
    ...
    if (!updatedProduct) {
      //caso o produto não seja encontrado ou atualizado
